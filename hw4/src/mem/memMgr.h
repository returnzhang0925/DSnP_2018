...
public:
	...
	void reset(size_t b = 0) {
		...
		{
			temp = _activeBlock->getNextBlock();
			delete _activeBlock;
			_activeBlock = temp;	
		}
		_activeBlock->reset();
...
private:
	...
	MemRecycleList<T>* getMemRecycleList(size_t n) {
		size_t m = n % R_SIZE;
		// TODO
		MemRecycleList<T>* temp_2 = &_recycleList[m];
		while(temp_2->getArrSize() != n)
		{
			if(temp_2->getNextList() != 0)
			{
				temp_2 = temp_2->getNextList();
			}
			else
			{
				temp_2->setNextList(new MemRecycleList<T>(n));
				return temp_2->getNextList();
			}
		}
		return temp_2;
	}
		...
		else
		{
			if(!_activeBlock->getMem(t, ret))
			{
				if (_activeBlock->getRemainSize() >= S)
				{
					...
					_activeBlock->getMem(downtoSizeT(_activeBlock->getRemainSize()), ret);
	...
	size_t getNumBlocks() const 
	{
		// TODO
		size_t count_2 = 0;
		MemBlock<T>* temp_3 = _activeBlock;
		while(temp_3)
		{
			temp_3 = temp_3->getNextBlock();
			count_2 ++;
		}
		return count_2;
	}
